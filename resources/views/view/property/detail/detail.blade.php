    ...
    <div class="flex gap-[25px] p-6 flex-col lg:flex-row">
        <div class="flex flex-col gap-[25px] w-full">
            <div class="flex lg:flex-row flex-col gap-[24px]">
                ...
                <x-box-dropdown class="w-[100%] lg:w-[70%]" name="Property Detail">
                    <div class="flex gap-[25px]">
                        <img src="{{ asset('storage/' . $property->album->imagePath) }}" alt="Cover Property" class="object-cover w-[200px] rounded-2xl h-[200px]">
                        <div class="flex flex-col gap-[5px]">
                            <div class="flex gap-[5px] items-center">
                                <x-a-label class="text-xl font-bold">Property Name</x-a-label>
                                <a href="{{ route('property.edit', ['id' => $property->id_property]) }}" class="bg-[#5E93DA] ml-2 py-[1px] text-sm px-[10px] text-white w-auto rounded-lg cursor-pointer">
                                    Edit Property
                                </a>
                            </div>
                ...
                <x-box-dropdown class="w-[100%] lg:w-[30%]" name="Reviews">
                    <div class="overflow-y-auto [&::-webkit-scrollbar]:w-2
                    [&::-webkit-scrollbar-track]:rounded-full
                    [&::-webkit-scrollbar-thumb]:rounded-full
                    [&::-webkit-scrollbar-thumb]:bg-[#5E93DA] h-[200px]">
                        <div class="flex flex-col justify-center p-4">
                            <div class="flex gap-[5px] items-center justify-center">
                                <x-a-label class="text-[58px] font-bold">{{ $property->getAvgRating() + 0 }}</x-a-label>
                        ...
                        <div class="flex gap-[15px] w-full flex-col pr-[10px]">
                            @foreach ($property->getComment as $comment)
                                <x-card.reviews fullName="{{ $comment->user->getFullName() }}" comment="{{ $comment->comment }}" rating="{{ $comment->rating }}" imgUrl="{{ $comment->user->getAvatarUrl() }}" 
                                    rentCover="{{ asset('storage/' . $comment->rent->album->imagePath) }}" rentName="{{ $comment->rent->rent_name }}" propertyId="{{ $property->id_property }}" rentId="{{ $comment->rent->id_rent }}"/>
                            @endforeach
                        </div>
                    </div>
                </x-nav-dropdown>
            ...
            <div class="grid gap-6 lg:grid-cols-1 xl:grid-cols-4">
                ...
                <x-box-dropdown name="Jumlah Fasilitas">
                 ...
                 <x-box-dropdown name="Lokasi">
                    ...
                    <div class="flex w-full">
                        <x-a-label class="text-lg truncate">{{ $property->property_address->street_name }}</x-a-label>
                    </div>
                </x-nav-dropdown>
                ...
                <x-box-dropdown name="Jumlah Booking">
                    ...
                    <div class="flex flex-col w-full gap-[15px]">
                        ...
                        <div class="flex gap-[10px]">
                            <div class="flex gap-[5px] bg-[#5E93DA] w-fit text-xs p-[3px] rounded-full px-[5px] max-h-[25px] items-center">
                                <div class="-rotate-90">
                ...
                <x-box-dropdown class="col-span-2 h-full" name="Manage">
                    <div class="flex justify-center items-center w-full h-full">
                        <div class="flex flex-wrap gap-[10px]">
                            <a href="{{ route('property.detail.calendar', $property->id_property) }}">
                                <x-primary-button class="w-auto h-[50px] !p-[10px] flex items-center gap-[10px]"><x-icon.calendar p="28" l="28"/>Calendar</x-primary-button>
                            </a>
                            <a href="{{ route('property.detail.rent.overview', $property->id_property) }}"
                                <x-primary-button class="w-auto h-[50px] !p-[10px] flex items-center gap-[10px]"><x-icon.rent p="28" l="28"/>Rents</x-primary-button>
                            </a>
                            <a href="{{ route('property.detail.task', $property->id_property) }}">
                                <x-primary-button class="w-auto h-[50px] !p-[10px] flex items-center gap-[10px]"><x-icon.task p="28" l="28"/>Tasks</x-primary-button>
                            </a>
                            <a href="{{ route('property.detail.transaction', $property->id_property) }}">
                                <x-primary-button class="w-auto h-[50px] !p-[10px] flex items-center  gap-[10px]"><x-icon.transaction p="28" l="28"/>Transactions</x-primary-button>
                            </a>
                            <a href="{{ route('property.detail.reservation', $property->id_property) }}">
                                <x-primary-button class="w-auto h-[50px] !p-[10px] flex items-center gap-[10px]"><x-icon.booking p="28" l="28"/>Reservations</x-primary-button>
                            </a>
                            <a href="{{ route('property.detail.iuran', $property->id_property) }}">
                                <x-primary-button class="w-auto h-[50px] !p-[10px] flex items-center gap-[10px]"><x-icon.iuran p="28" l="28"/>Iurans</x-primary-button>
                            </a>
                        </div>
                    </div>
                </x-nav-dropdown>
            ...
            <div id="unpaid-modal" class="flex hidden fixed inset-0 justify-center items-center bg-black bg-opacity-50">
                <div class="p-6 w-full max-w-md bg-white rounded-lg">
                    ...
                    <button onclick="closeModal('unpaid-modal')" class="py-2 mt-4 w-full text-white bg-red-500 rounded">Tutup</button>
                </div>
            </div>
            ...
            <div id="room-modal" class="flex hidden fixed inset-0 justify-center items-center bg-black bg-opacity-50">
                <div class="p-6 w-full max-w-md bg-white rounded-lg">
                    ...
                    <button onclick="closeModal('room-modal')" class="py-2 mt-4 w-full text-white bg-blue-500 rounded">Tutup</button>
                </div>
            </div>
        </div>
