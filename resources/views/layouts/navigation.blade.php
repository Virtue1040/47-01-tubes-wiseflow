...
<nav :class="{ 'block': openSidebar, 'hidden': !openSidebar }"
    class="float-left w-auto bg-white border-b border-gray-100 dark:bg-[#18181B] dark:border-gray-700">
        ...
        <div class="hidden sm:-my-px sm:mx-5 sm:flex sm:flex-col sm:gap-[10px]">
            @auth
                ...
                <x-nav-dropdown name="Booking">
                    ...
                    <x-nav-div :href="route('booking')" :active="request()->routeIs('order')" class="w-full">
                        <div class="flex gap-[15px] items-center mx-5  h-full p-[10px]">
                            <x-icon.box p="20" l="20" :active="request()->routeIs('order')"></x-icon.box>
                            <p>{{ __('Orders') }}</p>
                        </div>
